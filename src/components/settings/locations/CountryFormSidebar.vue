...
const formSchema = toTypedSchema(z.object({ name: z.string().min(1, 'Name is required').max(100) }))
const form = useForm({ validationSchema: formSchema })

watch(open, (val) => {
  if (val) form.resetForm({ values: { name: props.editItem?.name ?? '' } })
})
...
<template>
  <RightSidebar
    ...
    :description="isEdit ? 'Update the country name.' : 'Add a new country.'"
  >
    <form class="space-y-4" @submit.prevent="onSubmit">
      <FormField v-slot="{ componentField }" name="name">
        <FormItem>
          ...
          <FormControl>
            <Input v-bind="componentField" placeholder="e.g. Bangladesh" :disabled="loading" />
          </FormControl>
          <FormMessage />
        </FormItem>
      </FormField>
