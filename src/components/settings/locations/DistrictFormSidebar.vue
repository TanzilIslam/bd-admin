...
const emit = defineEmits<{
  submit: [values: { name: string; division_id: string }]
}>()
  ...
  if (val) {
    form.resetForm({
      values: {
        ...
        division_id: props.editItem?.division_id ?? props.defaultDivisionId ?? '',
      },
    })
  }
})
...
<template>
  <RightSidebar
    ...
    :description="isEdit ? 'Update the district details.' : 'Add a new district.'"
  >
