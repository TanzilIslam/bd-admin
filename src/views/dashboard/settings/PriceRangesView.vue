...
import type { PriceRangeItem } from '@/types'
import { usePriceRangesStore } from '@/stores/priceRangesStore'
import { useUI } from '@/composables/useUI'
import PriceRangeFormSidebar from '@/components/settings/price-ranges/PriceRangeFormSidebar.vue'
...
const store = usePriceRangesStore()
const { openDeleteConfirm } = useUI()

const sidebarOpen = ref(false)
const editItem = ref<PriceRangeItem | null>(null)
...
function openEdit(item: PriceRangeItem) {
  editItem.value = item
  sidebarOpen.value = true
}
...
function handleDelete(item: PriceRangeItem) {
  openDeleteConfirm({
...
const columnHelper = createColumnHelper<PriceRangeItem>()
  ...
  <div class="space-y-4">
    <div class="flex items-center justify-between">
      <div>
        ...
        <p class="text-sm text-muted-foreground">Manage price range tiers available to businesses.</p>
      </div>
    ...
    <div class="rounded-md border">
      <Table>
        ...
        <TableBody>
          ...
          <TableEmpty v-else :colspan="3">No price ranges found.</TableEmpty>
        </TableBody>
      </Table>
    </div>
  </div>

  <PriceRangeFormSidebar
